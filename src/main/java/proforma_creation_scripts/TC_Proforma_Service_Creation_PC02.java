...
public class TC_Proforma_Service_Creation_PC02 extends Keywords{
	...
	public void Proformacreation02(WebDriver driver) throws Exception{
	   ...
	   click(driver, edit_btn);
		...
		waitForElement5(driver, click_saveokbtn);
		
		String boundmessage =getText(driver, bound_errormsg);
		
		if(boundmessage.equals(Bound_Valueerror)) {
			System.out.println("Expected text is present");
		    Extent_pass(driver, "Expected text is present : "+bound_errormsg, test);
	    }else {
		System.out.println("Different text is not  present : "+bound_errormsg);
		Extent_fail(driver, "Different text is not  present : "+bound_errormsg, test);
	}  
