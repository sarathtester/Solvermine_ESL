...
public class TC_Service_Creation_SC06 extends Keywords {
	String url = TestNgXml.getdatafromExecution().get("TC_Service_Creation_SC06");
    ...
    String usedService=Utils.getDataFromTestData("ESL", "UsedService","Service_Creation");
 ...
 //Service creation	
 		...
 		if(isDisplayed(driver, Calender)) {
 			...
 			System.out.println("End Date Calender is Unavailable");
 			Extent_fail(driver, "End Date Calender is Unavailable", test);
 		}
 		...
 		waitForElement(driver, Popup_ServiceC);
...
//Delete service
   //For No
 		...
 		waitForElement(driver, servicePopup);
 		...
 		if(isDisplayed(driver,servicePopup)) {
 			...
 			System.out.println(deletPopupAct);
 			if(isDisplayed(driver, Success_Message))
 			{
 				
 				Extent_pass(driver, "Service delete popup displayed", test);
 				click(driver,deleteNo);
 			}
 			else
 			{
 				click(driver,confirmationPopup);
...
//Already Used Popup
 		...
 		waitForElement(driver, conditionFilter);
 		click(driver,conditionFilter);
 		sendKeys(driver,serviceCodeTextbox,usedService);
 		String actualUsedCode=getText(driver,selectService);
 		System.out.println(actualUsedCode);
 		if(actualUsedCode.equals(usedService))
 		{
		  Extent_pass(driver, "Searched value correctly displayed", test);
 		}else {
 			Extent_fail(driver, "Searched value incorrectly displayed", test);
 		...
 		if(isDisplayed(driver, alreadyUsedInFeeder)) {
 			Extent_pass(driver, "Service already used popup displayed", test);
 			click(driver,noButton);
 		} 		
 		}else {
 			Extent_fail(driver, "Service delete popup not displayed", test);
 		}
