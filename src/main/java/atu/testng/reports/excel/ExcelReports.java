...
public class ExcelReports {
	...
	public static void generateExcelReport(String paramString, List<ITestResult> paramList1,
		...
		try {
			...
			testSummary(localXSSFWorkbook, localSheet, paramList1, paramList2, paramList3);
			testCaseList(localXSSFWorkbook, localSheet, paramList1, paramList2, paramList3);
	...
	private static void addPieChartTOExcel(Workbook paramWorkbook, Sheet paramSheet, List<ITestResult> paramList1,
			List<ITestResult> paramList2, List<ITestResult> paramList3) {
		byte[] arrayOfByte = ExcelChart1.writeChartToExcel(paramList1.size(), paramList2.size(), paramList3.size());
	...
	private static void testSummary(Workbook paramWorkbook, Sheet paramSheet, List<ITestResult> paramList1,
			List<ITestResult> paramList2, List<ITestResult> paramList3) {
		...
		localCell.setCellValue(paramList1.size() + paramList2.size() + paramList3.size());
		ExcelStyler.setHeaderCellStyle(paramWorkbook, localCell);
	...
	private static void testCaseList(Workbook paramWorkbook, Sheet paramSheet, List<ITestResult> paramList1,
			List<ITestResult> paramList2, List<ITestResult> paramList3) {
	...
	private static int writeSummaryData(Workbook paramWorkbook, Sheet paramSheet, List<ITestResult> paramList,
			int paramInt) {
		paramList.size();
	...
	private static void testStepDescription(Workbook paramWorkbook, Sheet paramSheet, List<ITestResult> paramList) {
		...
		while (localIterator.hasNext()) {
			...
			while (((Iterator<?>) localObject).hasNext()) {
				...
				localCell.setCellValue(localSteps.getStatus());
				localCell = localRow2.createCell(8);
				try {
					Integer.parseInt(localSteps.getScreenShot().trim());
					CreationHelper localCreationHelper = paramWorkbook.getCreationHelper();
					Hyperlink localHyperlink = localCreationHelper.createHyperlink(4);
					String str2 = localITestResult.getAttribute("relativeReportDir") + Directory.SEP
			...
			localCell = ((Row) localObject).createCell(8);
			int j = paramSheet.addMergedRegion(new CellRangeAddress(i, i, 0, 8));
			localCell = paramSheet.getRow(i).getCell(j - 1);
			localCell.setCellStyle(
					ExcelStyler.setColor(paramWorkbook, ExcelStyler.FOOTER_BG_COLOR, ExcelStyler.FONT_COLOR));
		}
	}

	public static class ExcelChart1 {
