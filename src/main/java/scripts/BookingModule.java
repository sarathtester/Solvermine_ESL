...
public class BookingModule extends Keywords {
	
	/*String name = Utils.getDataFromTestData("DISTILL","Username","Sheet1");
	String moduleName=Utils.getDataFromTestData("BOOKING","ModuleName","Sheet1");
	String agrParty=Utils.getDataFromTestData("BOOKING","agrParty_l","Sheet1");*/
	...
	public void bookingModule(WebDriver driver)
	{	
...
scrollTop(driver);
scrollTop(driver);
verifyElementIsPresent(driver, EquipmentProfile);
...
WebElement dd5a1=	driver.findElement(By.id("mdl1_tablbl")); 
actions.moveToElement(dd5a1).perform();
verifyElementIsPresent(driver, Containeredit);
...
click(driver, Containerbookingupdateok);
		...
		//	Booking
			waitForElement(driver,searchBox_l);
			sendKeys(driver,searchBox_l,moduleName);
			enter(driver);
			sendKeys(driver,agrParty_l,agrParty);
			//sendKeys(driver,agrParty_l,Keys.TAB);
			Actions actions=new Actions(driver);
			actions.keyDown(ENTER);*/
